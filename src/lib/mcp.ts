...
import { firestoreConfigurado, getDoc, listDocs } from "./firestore-rest";
import { crearSolicitud, VALID_AREAS, VALID_KINDS, VALID_PRIORITIES } from "./request-intake";
import { checkRateLimit, getClientIp } from "./rate-limit";

/**
 * Servidor MCP de GanaPlay Diseño — el núcleo.
 *
 * Vive en `lib` y no en la ruta porque lo comparten DOS direcciones:
 *   · POST /api/mcp            → con `Authorization: Bearer <token>`
 *   · POST /api/mcp/t/<token>  → con el token en la propia dirección
 *
 * La segunda existe por una limitación real de los clientes: Claude Code y la
 * API dejan poner cabeceras, pero los "conectores" de claude.ai y de Claude
 * Desktop solo piden una URL. Sin esto, ese camino quedaba cerrado.
 *
...
/**
 * Versiones del protocolo que sabemos hablar.
 *
 * Se contesta con la MISMA que pida el cliente si está en esta lista. Antes se
 * respondía siempre la más nueva, y un cliente que hablaba una anterior podía
 * darse por incompatible y cortar la conexión.
 */
const VERSIONES_SOPORTADAS = ["2025-06-18", "2025-03-26", "2024-11-05"];
...
export async function responderMcp(req: Request, tokenDeLaRuta = "") {
  ...
  if (method === "initialize") {
    const pedida = String((cuerpo.params as Args | undefined)?.protocolVersion || "");
    return ok(id, {
      protocolVersion: VERSIONES_SOPORTADAS.includes(pedida) ? pedida : PROTOCOL_VERSION,
  ...
  // El token, por orden de preferencia: la cabecera (lo estándar y lo que
  // recomienda la especificación), y si no viene, el que traiga la dirección.
  const cabecera = req.headers.get("authorization") || "";
  const deCabecera = cabecera.replace(/^Bearer\s+/i, "").trim();
  const quien = verifyMcpToken(deCabecera || tokenDeLaRuta);
  if (!quien) {
    return rpcError(
      id,
      -32001,
  ...
  // Este servidor solo ofrece herramientas. Aun así contesta a los catálogos de
  // recursos y de prompts con una lista vacía en vez de "método no soportado":
  // varios clientes los piden igual al conectar y pintan un error rojo con lo
  // segundo, aunque la conexión esté perfecta.
